...
class Scanmem():
    ...
    def exec_command(self, strCmd):
        '''
            python strings are wchars, sm expects byte wide strings.
            therefore, we've got to recreate the string
        '''
        return backend.sm_execcommand(self.globals_ptr, strCmd.encode('utf-8'))
    ...
    def init(self):
        if self.are_commands_initialized:
            return False
        self.are_commands_initialized = bool(backend.sm_init())
        return self.are_commands_initialized
...
# bool sm_write_array(pid_t target, void *addr, const void *data, size_t len);
    def write_array(self, pidTarget, ptrAddress, ptrData, length):
        return backend.sm_write_array(pidTarget, ptrAddress, ptrData, length)
...
# } matches_and_old_values_array;
    ...
    def get_matches(self, is_string=False):
        '''
            Walks the matches_and_old_values_array the same way scanmem's own
            `list` command does (handlers.c: handler__list): swaths are laid
            out back-to-back, the array is terminated by a swath with a NULL
            first_byte_in_child, and every byte entry with a non-empty
            match_info is one match. Enumeration order here MUST stay
            identical to targetmem.c: nth_match(), as the indices are used
            with the `set N=value` command.

            For string scans, match_info holds the matched length rather than
            type flags (scanroutines.c: STRING routines), so decode the old
            bytes as text instead.
        '''
        matches = []
        base = self.globals.matches

        if not base or self.get_num_matches() == 0:
            return matches

        array_end = base + MatchArray.from_address(base).bytes_allocated
        swath_addr = base + sizeof(MatchArray)
        entry_size = sizeof(MatchInfo)
        match_index = 0

        while swath_addr + sizeof(MatchSwath) <= array_end:
            swath = MatchSwath.from_address(swath_addr)

            # A NULL first_byte_in_child terminates the array
            if not swath.first_byte_in_child:
                break

            data_addr = swath_addr + sizeof(MatchSwath)
            num_bytes = swath.number_of_bytes

            if data_addr + num_bytes * entry_size > array_end:
                break

            entries = (MatchInfo * num_bytes).from_address(data_addr)

            for i in range(num_bytes):
                match_info = entries[i].match_info

                if match_info == MatchFlag.FLAGS_EMPTY:
                    continue

                if is_string:
                    # match_info is the matched length; the string spans this
                    # entry plus the following (empty-flagged) placeholders
                    length = match_info
                    raw = bytes(entries[j].old_value
                                for j in range(i, min(i + length, num_bytes)))
                    value = raw.decode("ascii", errors="replace")
                    value_type = "string"
                    width = len(raw)
                else:
                    # Old value of up to the next 8 bytes, as data_to_val() does
                    raw = bytes(entries[j].old_value
                                for j in range(i, min(i + 8, num_bytes)))
                    value, value_type, width = decode_match_value(raw, match_info)

                address = swath.first_byte_in_child + i
                ...
                matches.append({
                    ...
                    "number_of_bytes": width,
                    "variable_bytes": list(raw[:width])
                })
                match_index += 1

            swath_addr = data_addr + num_bytes * entry_size
...
def decode_match_value(raw_bytes, match_info):
    '''
        Decode a match's old value, mirroring scanmem's data_to_val() width
        masking (targetmem.h) and valtostr() display priority (value.c).
        Returns (value, type_name, width_in_bytes).
    '''
    flags = match_info
    n = len(raw_bytes)
    if n < 8:
        flags &= ~MatchFlag.FLAGS_64B
    if n < 4:
        flags &= ~MatchFlag.FLAGS_32B
    if n < 2:
        flags &= ~MatchFlag.FLAGS_16B
    if n < 1:
        flags = MatchFlag.FLAGS_EMPTY

    int_widths = [
        (MatchFlag.FLAG_U64B, 8, False, "uint64"),
        (MatchFlag.FLAG_S64B, 8, True,  "int64"),
        (MatchFlag.FLAG_U32B, 4, False, "uint32"),
        (MatchFlag.FLAG_S32B, 4, True,  "int32"),
        (MatchFlag.FLAG_U16B, 2, False, "uint16"),
        (MatchFlag.FLAG_S16B, 2, True,  "int16"),
        (MatchFlag.FLAG_U8B,  1, False, "uint8"),
        (MatchFlag.FLAG_S8B,  1, True,  "int8"),
    ]

    for flag, width, signed, name in int_widths:
        if flags & flag:
            value = int.from_bytes(raw_bytes[:width], "little", signed=signed)
            return value, name, width

    if flags & MatchFlag.FLAG_F64B:
        return struct.unpack("<d", raw_bytes[:8])[0], "float64", 8
    if flags & MatchFlag.FLAG_F32B:
        return struct.unpack("<f", raw_bytes[:4])[0], "float32", 4

    return None, "unknown", 0


# Byte width of each concrete value type name (as reported by get_matches).
TYPE_WIDTHS = {
    "int8": 1, "uint8": 1,
    "int16": 2, "uint16": 2,
    "int32": 4, "uint32": 4,
...
def decode_typed_value(raw_bytes, value_type):
    '''
        Decode raw little-endian bytes as a known concrete type. Used for
        live re-reads where the exact type is already known. Strings are
        decoded as ASCII; unknown/short input returns None.
    '''
    if value_type == "string":
        return raw_bytes.split(b"\x00", 1)[0].decode("ascii", errors="replace")

    fmt = _STRUCT_FORMATS.get(value_type)
    width = TYPE_WIDTHS.get(value_type)
    if fmt is None or width is None or len(raw_bytes) < width:
        return None
    return struct.unpack(fmt, raw_bytes[:width])[0]


def parse_uservalue(input):
    '''
        Mirror of scanmem's parse_uservalue_number() (value.c): an integer
        input is also given float flags and vice versa, so that "auto"
        searches can match either representation, exactly like the CLI.
    '''
    val = UserValue()
    if parse_uservalue_int(input, val):
        val.flags |= MatchFlag.FLAGS_FLOAT
        if val.flags & MatchFlag.FLAG_S64B:
            num = val.int64_value
        else:
            num = val.uint64_value
        val.float32_value = float(num)
        val.float64_value = float(num)
        return val
    if parse_uservalue_float(input, val):
        num = val.float64_value
        int_ranges = [
            (0,         UINT8_MAX,  MatchFlag.FLAG_U8B,  "uint8_value"),
            (INT8_MIN,  INT8_MAX,   MatchFlag.FLAG_S8B,  "int8_value"),
            (0,         UINT16_MAX, MatchFlag.FLAG_U16B, "uint16_value"),
            (INT16_MIN, INT16_MAX,  MatchFlag.FLAG_S16B, "int16_value"),
            (0,         UINT32_MAX, MatchFlag.FLAG_U32B, "uint32_value"),
            (INT32_MIN, INT32_MAX,  MatchFlag.FLAG_S32B, "int32_value"),
            (0,         UINT64_MAX, MatchFlag.FLAG_U64B, "uint64_value"),
            (INT64_MIN, INT64_MAX,  MatchFlag.FLAG_S64B, "int64_value"),
        ]
        for low, high, flag, field in int_ranges:
            if low <= num <= high:
                val.flags |= flag
                # int() truncates towards zero, like the C casts
                setattr(val, field, int(num))
        return val
    return None
